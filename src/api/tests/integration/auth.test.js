...
describe('Authentication API', () => {
  ...
  beforeEach(async () => {
    ...
    user = {
      email: 'sudhanshusharma647@gmail.com',
      password: '123456',
    ...
    refresh_token = {
      ...
      user_id: '5947397b323ae82d8c3a333b',
  ...
  describe('POST /v1/auth/refresh-token', () => {
    it('should return a new accessToken when refreshToken and email match', async () => {
      ...
      return request(app)
        ...
        .send({ email: dbUser.email, refresh_token: refresh_token.token })
        .expect(httpStatus.OK)
        .then((res) => {
    ...
    it("should report error when email and refreshToken don't match", async () => {
      ...
      return request(app)
        .post('/v1/auth/refresh-token')
        .send({ email: user.email, refresh_token: refresh_token.token })
        .expect(httpStatus.UNAUTHORIZED)
        .then((res) => {
          const { code } = res.body;
          const { message } = res.body;
  ...
  describe('POST /v1/auth/send-password-reset', () => {
    it('should send an email with password reset link when email matches a user', async () => {
      const PasswordResetTokenObj = await PasswordResetToken.create(reset_token);

      expect(PasswordResetTokenObj.reset_token).to.be.equal('5947397b323ae82d8c3a333b.c69d0435e62c9f4953af912442a3d064e20291f0d228c0552ed4be473e7d191ba40b18c2c47e8b9d');
      expect(PasswordResetTokenObj.user_id.toString()).to.be.equal('5947397b323ae82d8c3a333b');
      expect(PasswordResetTokenObj.user_email).to.be.equal(dbUser.email);
    ...
    it("should report error when email doesn't match a user", async () => {
      await PasswordResetToken.create(reset_token);
      return request(app)
        .post('/v1/auth/send-password-reset')
  ...
  describe('POST /v1/auth/reset-password', () => {
    ...
    it('should report error when email is not provided', () => {
      return request(app)
        .post('/v1/auth/reset-password')
        .send({ password: 'updatedPassword', reset_token: reset_token.reset_token })
        .expect(httpStatus.BAD_REQUEST)
        .then((res) => {
          const field1 = res.body.errors[0].field;
          const location1 = res.body.errors[0].location;
    ...
    it('should report error when password is not provided', () => {
      return request(app)
        .post('/v1/auth/reset-password')
        .send({ email: dbUser.email, reset_token: reset_token.reset_token })
        .expect(httpStatus.BAD_REQUEST)
        .then((res) => {
          const field1 = res.body.errors[0].field;
          const location1 = res.body.errors[0].location;
    ...
    it('should report error when the resetToken is expired', async () => {
      ...
      expect(expiredPasswordResetTokenObj.reset_token).to.be.equal('5947397b323ae82d8c3a333b.c69d0435e62c9f4953af912442a3d064e20291f0d228c0552ed4be473e7d191ba40b18c2c47e8b9d');
      expect(expiredPasswordResetTokenObj.user_id.toString()).to.be.equal('5947397b323ae82d8c3a333b');
      expect(expiredPasswordResetTokenObj.user_email).to.be.equal(dbUser.email);
      ...
      return request(app)
        ...
        .send({
          ...
          reset_token: expiredResetToken.reset_token,
        })
        .expect(httpStatus.UNAUTHORIZED)
        .then((res) => {
          expect(res.body.code).to.be.equal(401);
